...
local function pulsemixer(args)
    local f = io.popen("pulsemixer "..args)
    if f == nil then
      return nil
    else
      local line = f:read("*all")
...
local function worker(format, sink)
    ...
    local vol = string.match(pulsemixer("--get-volume"), "%S+")
    return {vol, "on"}
end
-- }}}
...
function pulse_all.add(percent, sink)
    ...
    local cmd = string.format("pactl set-sink-volume %s %s%%", sink, percent)
...
function pulse_all.toggle(sink)
    ...
    local cmd = string.format("pactl set-sink-mute %s toggle", sink)
    return os.execute(cmd)
end
-- }}}
